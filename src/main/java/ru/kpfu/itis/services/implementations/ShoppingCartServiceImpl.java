...
import ru.kpfu.itis.services.ShoppingCartService;
...
public class ShoppingCartServiceImpl implements ShoppingCartService {
    ...
    public void addProduct(Product product) {
        if (products.containsKey(product)) products.replace(product, products.get(product) + 1);
    ...
    public void removeProduct(Product product) {
        if (products.containsKey(product)) {
            if (products.get(product) > 1) products.replace(product, products.get(product) - 1);
            else if (products.get(product) == 1) products.remove(product);
        }
    }
